...
def extract_city_landmark_map(docs) -> Dict[str, str]:
    ...
    for doc in docs:
        ...
        logger.info(f"Extracted text sample (first 100 chars): {text[:100]!r}")
    ...
    logger.info(f"City to landmark mapping: {mapping}")
    return mapping
...
def _get_favourite_city() -> Optional[str]:
    ...
    try:
        ...
        if not city:
            return None
        city_norm = CLEAN_RE.sub("", str(city)).strip()
        logger.info(f"Favorite city fetched: {city_norm}")
        return city_norm
    except Exception:
        return None

def fuzzy_match_landmark(landmark: str) -> Optional[str]:
    # Only if fuzzy matching available
    if not _HAS_FUZZY:
        return None

    candidates = {
        "getFirstCityFlightNumber": "gateway of india",
        "getSecondCityFlightNumber": "taj mahal",
        "getThirdCityFlightNumber": "eiffel tower",
        "getFourthCityFlightNumber": "big ben"
    }
    lm_norm = normalize_text(landmark)
    for endpoint, target in candidates.items():
        score = fuzz.partial_ratio(lm_norm, target)
        logger.info(f"Fuzzy matching '{lm_norm}' vs '{target}' score: {score}")
        if score > 80:
            return endpoint
    return None
...
def resolve_flight_number_from_docs(vector_store: Any, source_docs: List[Any]) -> Optional[Dict[str, str]]:
    try:
        ...
        if not mapping:
            logger.warning("No city-landmark mapping found")
            return {"favorite_city": None, "landmark": None, "flight_number": "Unknown"}

        fav_city_raw = _get_favourite_city()
        if not fav_city_raw:
            logger.warning("Favorite city not found")
            return {"favorite_city": None, "landmark": None, "flight_number": "Unknown"}

        fav_city_key = _normalize_city_key(fav_city_raw)
        landmark = mapping.get(fav_city_key)
        if not landmark:
            logger.warning(f"Landmark not found for city '{fav_city_raw}'")
            return {"favorite_city": fav_city_raw, "landmark": None, "flight_number": "Unknown"}

        logger.info(f"Landmark detected: '{landmark}'")

        # Try fuzzy matching first (if available)
        endpoint = None
        if _HAS_FUZZY:
            endpoint = fuzzy_match_landmark(landmark)

        # Fallback to normalized substring match
        if not endpoint:
            lm = normalize_text(landmark)
        ...
        logger.info(f"Flight number endpoint chosen: {endpoint}")
        ...
        logger.info(f"Flight number received: {flight}")

        return {"favorite_city": fav_city_raw, "landmark": landmark, "flight_number": str(flight).strip()}
    except Exception as e:
        logger.error(f"Error resolving flight number: {e}", exc_info=True)
        return {"favorite_city": None, "landmark": None, "flight_number": "Unknown"}
